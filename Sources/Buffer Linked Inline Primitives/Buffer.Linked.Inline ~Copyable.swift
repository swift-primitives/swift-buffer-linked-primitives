...
extension Buffer.Linked.Inline where Element: ~Copyable {
    ...
    public var count: Index<Element>.Count {
        borrowing get { header.count.retag(Element.self) }
    }

    /// Whether the list is empty.
    @inlinable
    public var isEmpty: Bool {
        borrowing get { header.count == .zero }
    }
...
extension Buffer.Linked.Inline where Element: ~Copyable {
    ...
    /// - Complexity: O(1)
    @usableFromInline
    mutating func _allocateSlot() throws(Self.Error) -> Index<Buffer<Element>.Linked<N>.Node>.Bounded<capacity> {
...
extension Buffer.Linked.Inline where Element: ~Copyable {
    public enum Insert {
        public typealias View = Property<Buffer<Element>.Linked<N>.Insert, Buffer<Element>.Linked<N>.Inline<capacity>>.Inout.Typed<Element>.Valued<N>.Valued<capacity>
    }

    public enum Remove {
        public typealias View = Property<Buffer<Element>.Linked<N>.Remove, Buffer<Element>.Linked<N>.Inline<capacity>>.Inout.Typed<Element>.Valued<N>.Valued<capacity>
    }
}

// MARK: - Property.Inout.Typed.Valued.Valued (.insert, .remove)
...
extension Buffer.Linked.Inline where Element: ~Copyable {
    @usableFromInline
    mutating func _insertFront(_ element: consuming Element) throws(Self.Error) {
    ...
    @usableFromInline
    mutating func _insertBack(_ element: consuming Element) throws(Self.Error) {
        let slot = try _allocateSlot()
        let sentinel = header.sentinel
    ...
    Element: ~Copyable
{
    ...
    public mutating func front(
        ...
        try base.value._insertFront(element)
    }
    ...
    public mutating func back(
        ...
        try base.value._insertBack(element)
    }
}
...
extension Property.Inout.Typed.Valued.Valued
where
    Tag == Buffer<Element>.Linked<n>.Remove,
    Base == Buffer<Element>.Linked<n>.Inline<m>,
    Element: ~Copyable
{
